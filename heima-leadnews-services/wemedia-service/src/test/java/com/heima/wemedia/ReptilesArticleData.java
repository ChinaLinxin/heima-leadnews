...
//                .get();
        ...
        for (Element article : articleList) {
            ...
            try {
                href = as.get(index).attr("href");
                if (index < as.size()){
                    index++;
                }
                System.out.println("新闻的url路径: " + href);
                Elements titleEle = article.getElementsByTag("h4");
                title = titleEle.text();
                System.out.println("新闻的文章标题:" + title);
                // 获取封面图片元素集合
                imgList = article.getElementsByClass("s-imgs");
                Elements img = article.getElementsByClass("s-right");
                if (img.size() >= 1) {
                    imgList = img;
                }
            ...
            for (Element imgEle : imgList) {
                Element img = imgEle.getElementsByTag("img").get(0);
...
//            String url = "//nimg.ws.126.net/?url=http%3A%2F%2Fdingyue.ws.126.net%2F2020%2F0227%2Fd5ba8ca7j00q6cggh000vc000hs00b7m.jpg&thumbnail=234x146&quality=85&type=jpg";
    ...
    private List<Map> parseContent(String href) {
        ...
        String url = "https://3g.163.com/ent" + href;
        ...
        try {
            document = Jsoup.connect(url)
                    ...
                    .get();
            Element contentDiv = document.getElementsByClass("main").get(0);
            Element article = contentDiv.getElementsByTag("article").get(0);
            Elements allElements = article.getElementsByTag("p");
